describe('seeker.state', function()
    ...
    describe('excluded files management', function()
        before_each(function()
            state.init()
        end)

        it('should initialize excluded_files to empty', function()
            assert.same({}, state.get_excluded_files())
        end)

        it('should add excluded files', function()
            state.add_excluded_files({ 'test1.lua', 'test2.lua' })
            local excluded = state.get_excluded_files()
            assert.equals(2, #excluded)
        end)

        it('should accumulate excluded files', function()
            state.add_excluded_files({ 'test1.lua' })
            state.add_excluded_files({ 'test2.lua' })
            local excluded = state.get_excluded_files()
            assert.equals(2, #excluded)
        end)

        it('should deduplicate excluded files', function()
            state.add_excluded_files({ 'test.lua' })
            state.add_excluded_files({ 'test.lua' })
            assert.equals(1, #state.get_excluded_files())
        end)

        it('should normalize paths in excluded files', function()
            state.add_excluded_files({ './test.lua' })
            state.add_excluded_files({ 'test.lua' })
            assert.equals(1, #state.get_excluded_files())
        end)

        it('should clear excluded files on init', function()
            state.add_excluded_files({ 'test1.lua', 'test2.lua' })
            state.init()
            assert.same({}, state.get_excluded_files())
        end)

        it('should clear excluded files', function()
            state.add_excluded_files({ 'test.lua' })
            state.clear_excluded_files()
            assert.same({}, state.get_excluded_files())
        end)

        it('should handle nil paths', function()
            state.add_excluded_files(nil)
            assert.same({}, state.get_excluded_files())
        end)

        it('should handle empty array', function()
            state.add_excluded_files({})
            assert.same({}, state.get_excluded_files())
        end)
    end)
end)
